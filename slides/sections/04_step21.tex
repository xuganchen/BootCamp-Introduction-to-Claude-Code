...
\vs
\begin{itemize}
    \item \ybb{This one you type.} Your own BDC, your own filing, your own criteria.
    \item You write one short file and the agent writes everything else.
    \item By the end you have a parser that foots to the balance sheet, plus a
          test suite that proves it.
\end{itemize}
\end{frame}
...
\begin{wideitemize}
  ...
  \item The instinct is to \yo{write more instruction}. That leaves you exactly
        where step 0 left you: prose is not checkable.
  \item \ybb{Put the human input where a machine can enforce it.}
        For example, ``zero nulls in the type field, tie-out within
        0.1 percent'' either holds or it does not.
\end{wideitemize}
\takeaway{Write down how you will know it worked before you write down what to build.}
\end{frame}

% Acceptance criteria come first.
\begin{frame}{Write down how you will know it worked, first}
\begin{columns}
...
You write \file{plan\_v0.md} yourself. Then, note the two constraints packed into
one sentence:
\begin{prompt}
...
\begin{wideitemize}
  ...
  \item \file{propose ... in @plan\_v1.md} makes the plan a \ybb{file you can edit},
        not a message you skim.
\end{wideitemize}
\end{frame}
...
\begin{center}
\includegraphics[width=\textwidth]{bdc_design_pharse1-2.png}
\end{center}
\end{frame}
...
% The folder structure the spec asked for.
\begin{frame}{Organized because the spec said so}
\begin{columns}
\begin{column}{0.3\textwidth}
\begin{center}
\includegraphics[width=\textwidth]{bdc_design_pharse1_folder_structure.png}
\end{center}
\end{column}
\begin{column}{0.65\textwidth}
\begin{itemize}
  \item \file{code/} the pipeline, \file{tests/} the checks, \file{output/}
        the deliverable.
  \item \file{note/} is the one folder that was not in plan\_v0.md, and it is the most useful:
        \file{trap\_log.md}, \file{parsing\_decisions.md}, \file{coverage.md}.
  \item \ybb{None of this is the agent being tidy.} It is the folder structure that
        was in plan\_v0.md.
\end{itemize}
\end{column}
\end{columns}
...
\begin{wideitemize}
    \item What it bought:
    \begin{itemize}
        \item A structured and staged pipeline: 2,757 lines, 10 scripts
        \item An independent test suite: 1,067 lines, reusable next quarter
        \item Criteria that catch the step 0 break: type never null, borrowers fewer than rows
        \item A gate that writes nothing on failure: on any failed check it exits non-zero and writes no output file.
    \end{itemize}
...
\begin{wideitemize}
  ...
  \item Run this parser, unchanged, across all 88: \yob{5 pass}.
\end{wideitemize}
\end{frame}
