...
\begin{itemize}
    \item Background: I travel between New Haven and Philadelphia constantly, and Amtrak is
        the only workable way to do it. The app is clunky, it does not talk to my
        calendar, \ybb{the ticket arrives once by email and sometimes it is gone}, and
        the pricing is dynamic.
    \item My old routine: dig each ticket out of my inbox, save it, and copy the trip
        into my calendar by hand; and often \yob{pay twice what the
        same seat had cost a week earlier}.
    \item So I built \ybb{TravelTab}, a personal Amtrak tracker that manages my trips \yob{automatically}:
    ...
    \end{enumerate}
    \item Now I just book a trip, and it's done. I can reach it \yob{from anywhere on any device}.
\end{itemize}
\end{frame}
...
% PetTab, 1 of 4: same stack, another domain.
...
% PetTab, 2 of 4: logging an event, the whole flow in three taps.
\begin{frame}{PetTab: easy to use in 3 clicks}
\begin{columns}[T]
\begin{column}{0.33\textwidth}
\centering
\includegraphics[height=0.8\textheight]{pettab_action1.png}\\[0.3em]
{\small \ybb{1. Tap what happened.}}
\end{column}
\begin{column}{0.33\textwidth}
\centering
\includegraphics[height=0.8\textheight]{pettab_action2.png}\\[0.3em]
{\small \ybb{2. Tap which cat.}}
\end{column}
\begin{column}{0.33\textwidth}
\centering
\includegraphics[height=0.8\textheight]{pettab_action3.png}\\[0.3em]
{\small \ybb{3. Logged, and editable.}}
\end{column}
\end{columns}
\end{frame}

% PetTab, 3 of 4: the data views.
\begin{frame}{PetTab: visualizing the data}
\begin{center}
\includegraphics[width=0.87\textwidth]{pettab_list.png}
\end{center}
\end{frame}

% PetTab, 4 of 4: an agent inside the product, on a read-only snapshot.
...
\begin{frame}{NanoClaw: an agent with an address}
\begin{columns}[T]
\begin{column}{0.5\textwidth}
\begin{center}
