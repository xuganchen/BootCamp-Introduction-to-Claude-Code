% Part 4: advanced usage examples. Divider plus 11 frames, session 2:36-3:00.
...
\begin{itemize}
    ...
    \item {\small \url{github.com/garrytan/gbrain}}
\end{itemize}
\end{column}
\end{columns}
\end{frame}

% TravelTab, 1 of 4: the background, as an annoyance.
\begin{frame}{TravelTab: personal-database management}
\begin{wideitemize}
  \item I travel between New Haven and Philadelphia constantly, and Amtrak is
...
% TravelTab, 2 of 4: the tickets dashboard.
\begin{frame}{TravelTab: personal-database management}
\begin{center}
...
% TravelTab, 3 of 4: the same records, a different question.
\begin{frame}{TravelTab: personal-database management}
\begin{center}
\includegraphics[height=0.8\textheight]{traveltab_tickets2.png}
\end{center}
\vspace{-0.2em}
This is what the database bought: it tells me how often I have been travelling.
\end{frame}

% TravelTab, 4 of 4: the fare watch, which runs unattended.
...
\vs
\begin{itemize}
    \item Background: I have two cats, Winnie and Dory, and I want to track their health and behaviour. Existing tools are too complicated, and I want full control over the data.
...
\begin{center}
\includegraphics[height=0.8\textheight]{pettab_list.png}
\end{center}
\end{frame}
...
\begin{center}
\includegraphics[height=0.75\textheight]{pettab_chat.png}
\end{center}
\end{column}
\begin{column}{0.55\textwidth}
\vs
\begin{itemize}
    \item The key difference: the agent is embedded in the product itself.
    \item PetTab's Insights chat runs the \file{claude} CLI over a
        \ybb{read-only snapshot} of the database, so that it can explore the recent behavior and health history of the cats, and give me suggestions on what to do next.
    \item Each chat gets its own working directory holding that read-only copy, restricted tools, a lock that
        serialises turns, and admin-only access.
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\begin{itemize}
    ...
    \item {\small\url{github.com/nanocoai/nanoclaw}}
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\begin{itemize}
    \item Try to use it in your own projects and daily life as much as possible. 
    \item Brainstorm your ideas, and then build them.
\end{itemize}
