...
\begin{column}{0.5\textwidth}
\begin{itemize}
  \item It \ybb{surveyed the filing} first: how many
  ...
  \item Then it chose the \ybb{XBRL tags} over the rendered tables -- a real
        judgment call, the right one for a 2026 filing.
...
a balance-sheet tie-out anyway. But, remember that \ybb{it} chose the check.
\end{frame}
...
\begin{wideitemize}
  ...
  \item If we stopped here, we would think the lesson was ``the prompt can be
        lazy''.
\end{wideitemize}
\end{frame}
...
\begin{frame}{Three models, three successes}
\tabfont
\begin{center}
