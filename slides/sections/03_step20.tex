...
\vs
\begin{wideitemize}
  \item Not in it: a ticker, a schema, a file format, a column list.
  \item Not in it: \yob{any word about verifying anything.}
\end{wideitemize}
\end{frame}

% It chose the BDC and the filing on its own.
\begin{frame}{It picked its own target}
\begin{columns}
\begin{column}{0.6\textwidth}
\begin{center}
\includegraphics[width=\textwidth]{bdc_design_pharse0-1.png}
\end{center}
\end{column}
\begin{column}{0.4\textwidth}
\begin{itemize}
  \item \ybb{Ares Capital (ARCC)}, reasoned from the words ``large publicly
        traded BDC''. 
  \item Its \ybb{most recent 10-Q}, quarter ended 2026-06-30: a 24.8 MB
...
% It inspects the document before parsing it.
\begin{frame}{It reads the document before parsing it}
\begin{columns}
\begin{column}{0.5\textwidth}
\begin{center}
\includegraphics[width=\textwidth]{bdc_design_pharse0-2.png}
\end{center}
\end{column}
\begin{column}{0.5\textwidth}
\begin{itemize}
  \item It did not open with a regex. It \ybb{surveyed the filing} first: how many
        tables, where the schedule starts, how the columns repeat.
  \item Then it chose the \ybb{XBRL tags} over the rendered tables -- a real
        judgment call, made unprompted, and the right one for a 2026 filing.
  \item ``\ybb{Look before you act}'' costs the agent a couple of minutes here, and
        it is the reason the parse works at all.
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\begin{itemize}
  ...
  \item Plus a footnote legend, a subtotals file, a README, and a
        \ybb{re-runnable parser} -- 803 lines of it.
  \item Clean CSVs you could hand to an analyst. \ybb{Twenty-two minutes,
...
a balance-sheet tie-out anyway. Remember that \ybb{it} chose the check, because
...
% Set up the reversal. This is genuinely good work.
\begin{frame}{This is genuinely good. Hold that thought.}
\begin{wideitemize}
  \item One sentence in. A working parser, two clean tables, a documented
        source-data bug, and a verification report out.
  \item \ybb{Every claim it made about its own output was true.}
  \item If you stopped here you would think the lesson was ``the prompt can be
        lazy''.
\end{wideitemize}
\end{frame}
...
\begin{wideitemize}
  \item Identical prompt, identical filing, three configurations:
  \begin{itemize}
    \item Opus 5 at xhigh effort (max > xhigh > high > medium > low)
    \item Opus 5 at medium effort
...
% All three passed their own check.
\begin{frame}{Three configurations, three successes}
\tabfont
\begin{center}
\begin{tabular}{p{5cm}p{8.0cm}}
\toprule
...
\end{tabular}
\end{center}
\takeaway{This is robust behavior, not a lucky run.}
\end{frame}
...
\end{tabular}
\end{center}
\takeaway{There are sixty of you in this room -- that is sixty correct, incompatible datasets and no pooled panel.}
\end{frame}
...
\end{shellout}
\begin{itemize}
    \item The problem: the instrument type text has been concatenated into the borrower name. It parsed the current period correctly and the comparative period wrongly.
...
\begin{wideitemize}
  \item \ybb{Sonnet 5 at medium effort is clean}: 593 and 580 distinct companies,
        one null type field, a third of xhigh's authoring budget.
  ...
  \item Richer and more auditable. Not more right on the dollars, which all three
        got right.
\end{wideitemize}
\takeaway{``Use the biggest model'' is not the lesson. Verification you designed is.}
\end{frame}
