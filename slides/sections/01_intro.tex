...
\vs
\begin{center}
\ylbb{Feedback is welcome -- now, or any time after today.}
\end{center}
\end{frame}
...
\begin{wideitemize}
  \item A \yob{chat window}. You describe the problem in prose, it hands back a
        snippet, you paste the snippet.
  \item It could not see your data, your files, or your errors. You were the
        clipboard between the model and the project.
  \item The code was \yob{roughly right and often not runnable}, which meant you
...
\begin{frame}{Working with AI, now: it acts in your project}
An agent that lives in your terminal, inside your project directory.
\vs
\begin{wideitemize}
  \item \ybb{Reads and writes files} in the repo -- your data, your code, your notes.
  \item \ybb{Runs commands} -- installs packages, executes scripts, reads the errors.
  \item \ybb{Iterates} -- sees its own output fail and tries again, without you retyping.
\end{wideitemize}
\takeaway{It does not hand you a snippet to paste. It acts in the project and \ybb{lives with the consequences.}}
\end{frame}
...
\begin{tikzpicture}[node distance=6mm]
  \node[flowbox_you]                (a) {instruct};
  \node[flowbox, right=of a]        (b) {it proposes\\and acts};
  \node[flowbox_you, right=of b]    (c) {you review};
  \node[flowbox, right=of c]        (d) {iterate};
  \draw[flowarrow] (a) -- (b);
  \draw[flowarrow] (b) -- (c);
  \draw[flowarrow] (c) -- (d);
  \draw[flowarrow] (d.south) -- ++(0,-0.7) -| (a.south);
  \node[flowlabel, above=1mm of a] {yours};
  \node[flowlabel, above=1mm of c] {yours};
\end{tikzpicture}
\end{center}
\vs
\begin{wideitemize}
  \item The two boxes with your name on them are \yob{the entire job}.
  \item Everything we add today -- planning, verification, generalization --
        ...
        \ybb{too large to read}.
\end{wideitemize}
\takeaway{Today you will sign off on a dataset of 37,197 rows without reading one of them.}
\end{frame}
...
\begin{center}
\yob{You will meet this four times this afternoon}, and it looks different every time.
\end{center}
\end{frame}
...
\textbf{4.} Advanced usage examples & demo and Q\&A \\
\bottomrule
\end{tabular}
\end{center}
\vs
...
% Install and access. Re-verify against docs.claude.com before teaching.
...
\end{center}
\vspace{-0.3em}
{\small \yob{If your terminal does not look like this, fix it now}. The rest of
...
% Context window: what you see against what the model sees.
\begin{frame}{Context window: You see two things, but the model sees all of it.}
\begin{center}
\includegraphics[width=0.7\textwidth]{context_window1.jpg}
\end{center}
\vspace{-0.6em}
{\small A ``chat'' is one long document passed back and forth. System prompt, tool
definitions, thinking, tool calls, results -- \ybb{all of it counts against your
window}, and you see almost none of it.}
...
\begin{wideitemize}
  ...
  \item \ybb{Compact deliberately.} \file{/compact} summarizes and continues;
        better still, have it write progress to disk first.
\end{wideitemize}
\end{column}
\end{columns}
\source{Source: A Series on Claude Code by Paul Goldsmith-Pinkham.}
...
\begin{wideitemize}
  ...
  \item \ybb{Everything ships:} every prompt as it was run, every transcript,
        every plan -- and the failures.
  \item \yob{No code is provided} -- not a downloader, not a parser, not a test.
        You build all of it with the agent.
\end{wideitemize}
\vs
\begin{center}
\file{github.com/xuganchen/BootCamp-Introduction-to-Claude-Code}
\end{center}
\end{frame}
