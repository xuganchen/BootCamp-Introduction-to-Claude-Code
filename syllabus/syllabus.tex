...
Assessment: & None. No homework, no grading. \\
\end{tabular}
...
The session builds on the pre-work (DataCamp modules and the Paul Goldsmith-Pinkham series). It opens with a short introduction to AI-assisted workflows and Claude Code, then turns to an in-class mini-project that applies agentic coding to a real asset-management task: building a private credit dataset from raw SEC filings and analyzing it. The project runs in four steps, and each step introduces the concept it needs (plan mode and sub-agents, skills, grounding and retrieval) at the point the work demands it, rather than as a separate lecture. From there we generalize those patterns beyond the project and cover AI safety. It closes with a few advanced-usage examples, most of them my own projects, to show what is possible beyond the basics.

% ---------- Prerequisite ------------------------------------------------------------------------------
\section*{Prerequisite}

Have a terminal agent installed and running before the session: we will work and experiment on the mini-project described below together. \href{https://github.com/xuganchen/BootCamp-Introduction-to-Claude-Code/blob/main/SETUP.md}{SETUP.md} in the course repository walks through it in about ten minutes and gives two options, one of them free for students. Claude Code is what will be on the screen; Google's Antigravity CLI also works. 

Setup instructions: \url{https://github.com/xuganchen/BootCamp-Introduction-to-Claude-Code/blob/main/SETUP.md}
...
\end{itemize}
Nothing executable is provided: you write the downloader, the parser and the verification check yourself, with the agent. That is the session. What you get instead is the target schema, the EDGAR ground rules, every prompt used in class, and the reference panels.
...
A sense of what is possible beyond the basics, time permitting:
\begin{itemize}
  \item \textbf{Personalized ``brain'':} indexed retrieval over a corpus you own, picking up where Step 3's grounding left off (GBrain, by Garry Tan, \url{https://github.com/garrytan/gbrain}).
  \item \textbf{Personal database management:} \emph{TravelTab}, a self-hosted Amtrak ticket tracker that auto-imports tickets from email, exports structured records to a searchable dashboard, syncs to a calendar, and watches fares on a schedule. The ingest is this session's project in miniature: messy documents in, structured records out.
